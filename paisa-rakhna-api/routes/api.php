...
Route::middleware('auth:sanctum')->post('/mpin/otp', function (\Illuminate\Http\Request $request) {
    ...
    return response()->json(['success' => true, 'message' => 'OTP sent', 'otp' => (config('app.debug') && app()->environment('local')) ? $code : null]);
});
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::middleware('throttle:20,1')->post('/transactions/send', [TransactionController::class, 'send']);

    // M-PIN
    Route::get('/mpin/status',   [MpinController::class, 'status']);
    Route::post('/mpin/set',     [MpinController::class, 'set']);
    Route::middleware('throttle:10,1')->post('/mpin/verify',  [MpinController::class, 'verify']);
