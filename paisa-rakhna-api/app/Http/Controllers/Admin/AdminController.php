...
class AdminController extends Controller
{
    public function dashboard()
    {
        $stats = [
            ...
            'kyc_under_review'   => KycDocument::where('status', 'under_review')->count(),
            'kyc_verified'       => KycDocument::where('status', 'verified')->count(),
        ];
    ...
    public function walletAdjust(Request $request, User $user)
    {
        ...
        if ($request->action === 'deduct' && $wallet->balance < $amount) {
            return back()->with('error', 'Cannot deduct more than current balance (PKR ' . number_format((float) $wallet->balance, 2) . ').');
        }
    ...
    {
        if ($user->is_admin) {
            return back()->with('error', 'Cannot edit admin accounts.');
        }
    ...
    public function rates()
    {
        ...
        $current = Cache::get('zakat_rates', [
            'gold_rate_per_tola'   => 245000,
            'silver_rate_per_tola' => 2800,
            'source'               => 'static',
        ]);

        return view('admin.rates', compact('current', 'override'));
    }

    public function updateRates(Request $request)
    {
        $request->validate([
            'action'               => 'required|in:override,clear',
            'gold_rate_per_tola'   => 'required_if:action,override|nullable|numeric|min:1',
            'silver_rate_per_tola' => 'required_if:action,override|nullable|numeric|min:1',
        ]);

        if ($request->action === 'clear') {
            Cache::forget('admin_rates_override');
            Cache::forget('zakat_rates');
            return back()->with('success', 'Custom rates cleared. Live rates will be used.');
        }

        $override = [
            'gold_rate_per_tola'   => (int) $request->gold_rate_per_tola,
            'silver_rate_per_tola' => (int) $request->silver_rate_per_tola,
        ...
        Cache::forever('admin_rates_override', $override);
        Cache::forever('zakat_rates', $override);

        return back()->with('success', 'Custom rates saved and applied.');
    }
