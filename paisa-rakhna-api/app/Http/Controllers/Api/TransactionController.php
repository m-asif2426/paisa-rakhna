...
class TransactionController extends Controller
{
    ...
    public function send(Request $request)
    {
        $request->validate([
            ...
            'amount'          => 'required|numeric|min:1|max:9999999|regex:/^\d+(\.\d{1,2})?$/',
        ]);
        ...
        $recipient = User::with('wallet')->where('phone', $recipientPhone)->first();
        ...
        $ref = 'TXN-' . now()->format('Ymd') . '-' . strtoupper(bin2hex(random_bytes(3)));

        DB::transaction(function () use ($sender, $recipient, $request, $recipientPhone, $ref) {
