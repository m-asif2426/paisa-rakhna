...
class AuthController extends Controller
{
    ...
    public function sendOtp(Request $request)
    {
        ...
        // Always generate a real 6-digit random OTP (no hardcoded values)
        ...
        if ($channel === 'email' && $email) {
            ...
            // Queue email for background delivery (instant API response)
            try {
                Mail::to($email)->queue(new OtpMail($code, $purpose));
            } catch (\Throwable $e) {
                \Log::error('OTP email queue failed: ' . $e->getMessage());
            }
        } else {
        ...
        return response()->json([
            ...
            // Only return OTP value in debug-explicitly-enabled local dev (APP_DEBUG=true)
            'otp'         => (config('app.debug') && app()->environment('local')) ? $code : null,
        ]);
    }
