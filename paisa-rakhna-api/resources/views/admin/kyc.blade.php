            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @endif
                        <form method="POST" action="{{ route('admin.kyc.reset', $doc) }}" class="d-inline"
                            onsubmit="return confirm('Reset KYC for {{ addslashes($doc->user->name ?? '') }}?')">
                            @csrf
                            <button type="submit" class="btn btn-sm btn-outline-warning px-2 py-1" style="font-size:12px; border-radius:6px;" title="Reset to pending">
