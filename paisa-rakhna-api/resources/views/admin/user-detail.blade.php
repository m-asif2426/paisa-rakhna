...
<div class="mb-3 d-flex gap-2">
    ...
    <a href="{{ route('admin.users.edit', $user) }}" class="btn btn-sm btn-warning" style="border-radius:8px; font-weight:600;">
            ...
            <div class="p-3">
                ...
                <div class="mt-3 d-flex gap-2 flex-wrap">
                    ...
                    <button type="button" class="btn btn-sm btn-danger" style="border-radius:8px;"
                        onclick="document.getElementById('rejectForm-{{ $kyc->id }}').classList.toggle('d-none')">
                        <i class="bi bi-x-lg me-1"></i>Reject
                    </button>
                    @endif
                    <form method="POST" action="{{ route('admin.kyc.reset', $kyc) }}"
                        onsubmit="return confirm('Reset KYC for {{ addslashes($kyc->user->name) }}? User will need to resubmit.')">
                        @csrf
                        <button type="submit" class="btn btn-sm btn-outline-warning" style="border-radius:8px;">
                            <i class="bi bi-arrow-counterclockwise me-1"></i>Reset KYC
                        </button>
                    </form>
                </div>

                {{-- Reject form (hidden by default) --}}
                <form id="rejectForm-{{ $kyc->id }}" method="POST" action="{{ route('admin.kyc.reject', $kyc) }}"
                    class="mt-2 d-none">
                    @csrf
                    <input type="text" name="reason" class="form-control form-control-sm mb-2"
                        placeholder="Rejection reason (required)" required style="border-radius:8px;">
